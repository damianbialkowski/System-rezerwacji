    ...
    <div class="newsletter">
        <div class="container-fluid">
            <div class="row align-items-center">
                ...
                <div class="col-md-6">
                    @if(isset($users) && $users->isSubscribeNewsletter())
                        <p class="text-center">Zapisałeś/aś się już do Newsletter'a!</p>
                    @else
                    <form action="{{ url('newsletter/subscribe') }}" method="GET">
                        @csrf
                        <div class="input-group">
                        <input type="email" class="form-control" name="email" placeholder="Adres e-mail" @if(\Auth::check() && isset($users) && $users->email) value="{{$users->email}}"  @endif>
